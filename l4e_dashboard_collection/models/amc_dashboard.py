...
class AmcDashboard(models.TransientModel):
    ...
    def get_dashboard_data(
        ...
        if has_planned_date and has_deadline:
            schedule_domain += ['|', ('planned_date_begin', '!=', False), ('date_deadline', '!=', False)]
        elif has_planned_date:
        ...
        overdue_tasks = all_tasks.filtered(
            ...
            and ((t.planned_date_begin and _to_date(t.planned_date_begin) < today) or (has_deadline and t.date_deadline and _to_date(t.date_deadline) < today))
        ...
        for task in schedule_tasks:
            ...
            if task_date:
                d_obj = _to_date(task_date)
                if d_obj:
                    monthly_actions[d_obj.month].append(task.id)

        scheduled_services = []
        display_tasks = all_tasks.sorted(
            key=lambda t: _to_date(t.planned_date_begin if has_planned_date and t.planned_date_begin else (t.date_deadline if has_deadline and t.date_deadline else t.create_date)) or date.min,
            reverse=False
        )[:50]
