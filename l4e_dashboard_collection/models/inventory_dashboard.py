    ...
    def get_dashboard_data(self, date_from=None, date_to=None, product_id=None):
        ...
        self.env.cr.execute(f"""
            ...
            WHERE svl.company_id = %s
              ...
              {prod_cond}
        """, tuple(val_params))
        total_value = self.env.cr.fetchone()[0]
        ...
        prev_total_value = self.env.cr.fetchone()[0]

        prod_count_domain = [('categ_id.show_in_dashboard', '=', True), ('active', '=', True)]
        if product_id:
        ...
        self.env.cr.execute(f"""
            ...
            WHERE pc.show_in_dashboard = TRUE AND pp.active = TRUE
        ...
        self.env.cr.execute(f"""
            ...
            WHERE sm.state = 'done'
              ...
              {prod_cond}
        """, tuple(in_out_params))
        current_out = self.env.cr.fetchone()[0]
        ...
        self.env.cr.execute(f"""
            ...
            WHERE sm.state = 'done'
              ...
              {prod_cond}
        """, tuple(prev_in_out_params))
        prev_out = self.env.cr.fetchone()[0]
        ...
        running_val = self.env.cr.fetchone()[0]
        ...
        self.env.cr.execute(f"""
            ...
            ORDER BY val DESC
        """, tuple(cat_params))
        category_vals = [{'id': row[0], 'name': row[1], 'value': round(row[2], 2)} for row in self.env.cr.fetchall()]

        top_prod_params = [lang, company_id, dt_to_utc]
        if product_id:
            top_prod_params.append(product_id)
        self.env.cr.execute(f"""
            SELECT pp.id, COALESCE(pt.name->>%s, pt.name->>'en_US') AS name, COALESCE(SUM(svl.value), 0.0) AS val, pt.id AS tmpl_id
            FROM stock_valuation_layer svl
            JOIN product_product pp ON pp.id = svl.product_id
            JOIN product_template pt ON pt.id = pp.product_tmpl_id
            JOIN product_category pc ON pc.id = pt.categ_id
        ...
        self.env.cr.execute(f"""
            SELECT sl.id, sl.complete_name, COALESCE(SUM(sq.quantity), 0.0) AS qty,
            ...
            LIMIT 5
        """, tuple(loc_params))
        location_vals = [{'id': row[0], 'name': row[1], 'qty': round(row[2], 2), 'value': round(row[3], 2)} for row in self.env.cr.fetchall()]
        ...
        self.env.cr.execute(f"""
            ...
            SELECT pp.id, COALESCE(pt.name->>%s, pt.name->>'en_US') AS name,
