...
class InventoryPastCount(models.Model):
    ...
    def _apply_count(self):
        ...
        if float_is_zero(difference, precision_rounding=rounding):
            ...
            if self.cost_price:
                self.env.cr.execute(
                    "SELECT SUM(product_qty), SUM(value) FROM stock_move "
                    "WHERE product_id = %s AND company_id = %s AND state = 'done' AND date <= %s",
                ...
                if not float_is_zero(value_diff, precision_rounding=0.01):
                    ...
                    # 1. Create 0-quantity stock.move in Odoo 19 for value tracking
                    reval_move_vals = {
                        'name': _("Product Quantity Updated [Accounted on %s]", self.counted_date),
                        'product_id': self.product_id.id,
                    ...
                    reval_move = self.env['stock.move'].sudo().create(reval_move_vals)

                    # Accounting entry in Odoo 19
                    categ = self.product_id.categ_id
                    if hasattr(categ, 'property_valuation') and categ.property_valuation in ('real_time', 'perpetual'):
                        ...
                        if stock_journal and acc_valuation and acc_variation:
                            ...
                            if reval_move:
                                reval_move.sudo().write({'account_move_id': account_move.id})

            self.write({
                'state': 'done',
                'location_id': location.id,
                'move_id': reval_move.id if reval_move else False,
    ...
    def _backdate_created_records(self, moves, location, target_datetime,
        ...
        account_move_ids = [mid for target in targets.values()
                            for mid in target.get('account_move_ids', [])]
        if not account_move_ids:
            account_moves = moves.sudo().mapped('account_move_id')
            if account_moves:
                account_move_ids = account_moves.ids
        
        applied_value = sum(moves.sudo().mapped('value'))
        primary = moves[0]
        ...
        self.write({
            ...
            'valuation_layer_count': len(moves),
    ...
    def _backdate_update_product_cost(self):
        ...
        # In Odoo 19: Total valuation value across all done stock.moves
        self.env.cr.execute(
            "SELECT SUM(value) FROM stock_move "
            "WHERE product_id = %s AND company_id = %s AND state = 'done'",
