...
class UniversalFieldUpdateLog(models.Model):
    ...
    state = fields.Selection([
        ('completed', 'Completed'),
        ('reverted', 'Reverted'),
    ], string="Status", default='completed', readonly=True, copy=False)
    ...
    global_batch_id = fields.Char(string="Global Batch", readonly=True, copy=False, index=True)
    is_delete_log = fields.Boolean(string="Is Delete Log", default=False, readonly=True, copy=False)
    deleted_res_id = fields.Integer(string="Deleted Record ID", readonly=True, copy=False)
    deleted_res_name = fields.Char(string="Deleted Record Name", readonly=True, copy=False)
    deleted_snapshot_json = fields.Text(string="Deleted Record Snapshot", readonly=True, copy=False)
    ...
    def _action_revert_delete(self):
        self.ensure_one()
        if self.state == 'reverted':
            raise UserError(_("This deletion has already been reverted."))
        if not self.deleted_snapshot_json:
            raise UserError(_("No snapshot was stored for this deletion — cannot restore."))

        snapshot = json.loads(self.deleted_snapshot_json)
        Model = self.env.get(self.model_name)
        if Model is None:
            raise UserError(_("The target model '%s' no longer exists.") % self.model_name)

        self.env['universal.field.updater']._restore_record_snapshot(
            self.model_name, self.deleted_res_id, snapshot)

        self.sudo().write({'state': 'reverted'})

        # Also revert every sibling log from the same global-replace batch
        # that's still completed, so references point back to the restored
        # record in the same action — no need to click Revert N times, and
        # in the right order (record exists again before refs are rewritten).
        sibling_reverted = 0
        sibling_failed = 0
        if self.global_batch_id:
            ...
            for sibling in siblings:
                try:
                    with self.env.cr.savepoint():
                        sibling.action_revert_update()
                    sibling_reverted += 1
                except Exception:
                    sibling_failed += 1

        message = _("Record restored (ID %s).") % self.deleted_res_id
        if self.global_batch_id:
            message += " " + _("%(r)s related reference log(s) also reverted, %(f)s failed.") % {
                'r': sibling_reverted, 'f': sibling_failed,
            }
        ...
        return {
            ...
            'params': {
                'title': _('Deletion Reverted'),
                'message': message,
                'type': 'success' if sibling_failed == 0 else 'warning',
                'sticky': True,
                'next': {
    ...
    def action_revert_update(self):
        ...
        if self.is_delete_log:
            return self._action_revert_delete()
...
class IrModelFields(models.Model):
    ...
    def name_get(self):
        if self.env.context.get('show_tech_name'):
            return [(rec.id, f"{rec.field_description} ({rec.name})") for rec in self]
