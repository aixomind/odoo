...
DynamicRecordSelector.supportedTypes = ["char"];
DynamicRecordSelector.extractProps = ({ attrs, options }) => {
    const opts = options || (attrs && attrs.options) || {};
    return {
        relationField: opts.relation_field || "relation_model",
    };
};

registry.category("fields").add("dynamic_record_selector", DynamicRecordSelector);