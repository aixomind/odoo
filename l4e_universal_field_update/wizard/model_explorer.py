...
class UniversalFieldUpdater(models.TransientModel):
    ...
    delete_old_after_replace = fields.Boolean(
        string="Delete Old Record After Replace",
        default=False,
        help="After every reference has been repointed to the replacement "
             "record, deletes the old (now-orphaned) record. Only deletes "
             "if there were zero failures AND a final re-check finds no "
             "remaining references anywhere in the scan results — otherwise "
             "the old record is left in place and the result message says "
             "why, so nothing is silently orphaned. Note: 'Scan All Modules' "
             "only finds many2one/many2many fields — it cannot see Reference "
             "fields, attachments, or other non-relational pointers, so "
             "deletion can still fail with a database error if something "
             "outside that scan still points to this record. Only applies "
             "to Global mode."
    )
    ...
    def _capture_record_snapshot(self, record):
        """Raw-SQL snapshot of a record's own table row, plus any many2many
        pivot rows it participates in, so it can be fully restored later —
        including with its original ID, which is what makes references we
        can't see (Reference fields, attachments, etc.) valid again too."""
        table = record._table
        self.env.cr.execute('SELECT * FROM "%s" WHERE id = %%s' % table, (record.id,))
        ...
        m2m_data = []
        for fname, f in record._fields.items():
            if f.type == 'many2many' and f.store:
                rel_table, col1, col2 = f.relation, f.column1, f.column2
                self.env.cr.execute(
                    'SELECT "%s" FROM "%s" WHERE "%s" = %%s' % (col2, rel_table, col1),
                    (record.id,)
                )
                other_ids = [r[0] for r in self.env.cr.fetchall()]
                if other_ids:
                    m2m_data.append({'table': rel_table, 'col1': col1, 'col2': col2, 'other_ids': other_ids})

        return {'table': table, 'columns': col_names, 'row': row_data, 'm2m': m2m_data}

    @api.model
    def _restore_record_snapshot(self, model_name, res_id, snapshot):
        """Counterpart to _capture_record_snapshot — reinserts the row with
        its original ID and re-populates its own many2many pivot rows."""
        Model = self.env[model_name]
        if Model.browse(res_id).exists():
            raise UserError(_("A record with ID %s already exists — cannot restore over it.") % res_id)
        ...
        for m2m in snapshot.get('m2m', []):
            for other_id in m2m['other_ids']:
                self.env.cr.execute(
                    'INSERT INTO "%s" ("%s", "%s") VALUES (%%s, %%s) ON CONFLICT DO NOTHING'
                    % (m2m['table'], m2m['col1'], m2m['col2']),
                    (res_id, other_id),
                )
    ...
    def action_global_scan(self):
        ...
        for f in candidate_fields:
            ...
            if getattr(Model, '_auto', True) is False:
                # SQL-view-backed reporting models (sale.report,
                # report.stock.quantity, etc.) aren't real tables — Postgres
                # will always reject writing to them, so including them here
                # only produces guaranteed failures at apply time.
                continue
    ...
    def action_global_apply(self):
        ...
        if self.delete_old_after_replace:
            if total_failed:
                delete_note = " " + _("Old record was NOT deleted: %s reference(s) failed to update.") % total_failed
            else:
                # Final safety net: re-check every field the scan found (not
                # just the lines selected for this run) for any remaining
                # reference to old_id before deleting anything.
                remaining = 0
                for gline in self.global_line_ids:
                    Model = self.env.get(gline.model_name)
                    if Model is None:
                        continue
                    try:
                        with self.env.cr.savepoint():
                            if gline.field_ttype == 'many2one':
                                remaining += Model.sudo().search_count([(gline.field_name, '=', old_id)])
                            else:
                                remaining += Model.sudo().search_count([(gline.field_name, 'in', [old_id])])
                    except Exception:
                        remaining += 1  # can't verify -> treat as unsafe

                if remaining:
                    delete_note = " " + _("Old record was NOT deleted: %s reference(s) still point to it.") % remaining
                else:
                    try:
                        old_record = self.env[self.global_model_name].sudo().browse(old_id)
                        snapshot = self._capture_record_snapshot(old_record)
                        with self.env.cr.savepoint():
                            # Always go through the ORM here, even if force_bypass
                            # was used for the rewiring above — unlink() runs each
                            # model's own deletion safeguards (e.g. res.partner
                            # blocking deletion of a partner with invoices), which
                            # matter more on delete than anywhere else in this tool.
                            old_record.unlink()
                        deleted = True
                        delete_note = " " + _("Old record (%s) was deleted.") % old_display

                        delete_log = LogModel.create({
                            'model_name': self.global_model_name,
                            'model_label': self.global_model_id.name,
                            'field_name': 'id',
                            'field_label': _('Record Deletion'),
                            'field_type': 'many2one',
                            'relation_model': self.global_model_name,
                            'old_value_display': old_display,
                            'new_value_display': _("(deleted)"),
                            'old_value_raw': str(old_id),
                            'new_value_raw': '',
                            'requested_count': 1,
                            'updated_count': 1,
                            ...
                            'failed_count': 0,
                            'is_delete_log': True,
                            'deleted_res_id': old_id,
                            'deleted_res_name': old_display,
                            'deleted_snapshot_json': json.dumps(snapshot),
                            'global_batch_id': batch_id,
                        })
                        log_ids.append(delete_log.id)
                    except Exception as e:
                        delete_note = " " + _("Old record was NOT deleted: %s") % str(e)
        ...
        return {
            ...
            'params': {
                ...
                'message': message,
                'type': 'success' if (total_failed == 0 and (not self.delete_old_after_replace or deleted)) else 'warning',
                'sticky': True,
                'next': {
                    'type': 'ir.actions.act_window',
                    'res_model': 'universal.field.update.log',
