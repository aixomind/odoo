...
class MailThreadInherit(models.AbstractModel):
    ...
    def _track_template(self, changes):
        ...
        if ir_model:
            ...
            if config and config.tracking_enable and not config.remove_tracking:
                return {}
        return super(MailThreadInherit, self)._track_template(changes)

    def _track_finalize(self):
        """Odoo 16 override: Intercept the precommit tracking hook to post
        custom green card messages instead of standard bullet-format tracking
        messages. This handles ALL tracked fields including stored computed
        fields like amount_total that are not captured by write() overrides."""
        ir_model = self.env['ir.model'].sudo().search(
            [('model', '=', self._name)], limit=1
        )
        if not ir_model:
            return super(MailThreadInherit, self)._track_finalize()
        config = self.env['tracking.config'].sudo().search(
            [('model_id', '=', ir_model.id)], limit=1
        )
        if not (config and config.tracking_enable and not config.remove_tracking):
            return super(MailThreadInherit, self)._track_finalize()

        # Pop Odoo 16's internal precommit tracking data so no standard
        # bullet-format messages are posted by the base implementation.
        tracking_key = 'mail.tracking.%s' % self._name
        tracking_data = self.env.cr.precommit.data.pop(tracking_key, {})

        for record_id, initial_values in tracking_data.items():
            try:
                record = self.browse(record_id).exists()
                if not record:
                    continue
                for fname, old_val in initial_values.items():
                    ...
                    if field.type == 'boolean':
                        old_str = _('Yes') if old_val else _('No')
                        new_str = _('Yes') if new_val else _('No')
                        if old_str != new_str:
                            msg = _format_tracking_message(field_string, old_str, new_str)
                            record.with_context(bypass_chatter_tracking=True).message_post(body=msg)
                    elif field.type == 'many2one':
                        old_str = old_val.display_name if old_val else False
                        new_str = new_val.display_name if new_val else False
                        if old_str != new_str:
                            msg = _format_tracking_message(field_string, old_str, new_str)
                            record.with_context(bypass_chatter_tracking=True).message_post(body=msg)
                    elif field.type in ('many2many', 'one2many'):
                        old_name_map = {r.id: r.display_name for r in (old_val or [])}
                        ...
                        old_str = str(old_val) if old_val is not False and old_val is not None else False
                        new_str = str(new_val) if new_val is not False and new_val is not None else False
                        if old_str != new_str:
                            msg = _format_tracking_message(field_string, old_str, new_str)
                            record.with_context(bypass_chatter_tracking=True).message_post(body=msg)
            except Exception as e:
                _logger.error(
                    "Custom _track_finalize error for %s[%s]: %s",
                    self._name, record_id, str(e)
                )
        # Return without calling super() to suppress standard tracking messages
            ...
            for record in self:
                ...
                for fname in vals.keys():
                    if fname in record._fields:
                        field = record._fields[fname]
                        # Skip fields with tracking=True — _track_finalize handles those
                        # (covers both direct writes and computed field changes)
